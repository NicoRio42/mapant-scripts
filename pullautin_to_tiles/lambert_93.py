...
def make_max_zoom_tiles():
    ...
    for png in png_files:
        ...
        if png_width == PULLAUTIN_PIXEL_SIZE and png_height == PULLAUTIN_PIXEL_SIZE:
            ...
            filled_png = im_hp.fill_small_png_with_alpha(png_path, pgw_path, PULLAUTIN_PIXEL_SIZE, 500, tile_geometry)
        ...
        for y in y_list:
            ...
            if not os.path.exists(parent_path):
                ...
                if os.path.exists(x0_y0_path):
                    ...
                    x0_y0 = im_hp.create_transparent_image(TILE_PIXEL_SIZE, TILE_PIXEL_SIZE)
                
                if os.path.exists(x1_y0_path):
                    x1_y0 = Image.open(x1_y0_path)
                else:
                    x1_y0 = im_hp.create_transparent_image(TILE_PIXEL_SIZE, TILE_PIXEL_SIZE)
                
                if os.path.exists(x0_y1_path):
                    x0_y1 = Image.open(x0_y1_path)
                else:
                    x0_y1 = im_hp.create_transparent_image(TILE_PIXEL_SIZE, TILE_PIXEL_SIZE)
                
                if os.path.exists(x1_y1_path):
                    x1_y1 = Image.open(x1_y1_path)
                else:
                    x1_y1 = im_hp.create_transparent_image(TILE_PIXEL_SIZE, TILE_PIXEL_SIZE)
                ...
                merged_resized = merged.resize((TILE_PIXEL_SIZE, TILE_PIXEL_SIZE))
...
def make_big_zoom_tiles(zoom):
    ...
    for x in x_list:
        ...
        for y in y_list:
            ...
            children_tiles = get_children_tiles(int(x), int(os.path.splitext(y)[0]))
            x0_y0_path = os.path.join(children_tiles_path, str(children_tiles[0][0]), str(children_tiles[1][0]) + ".png")
            x1_y0_path = os.path.join(children_tiles_path, str(children_tiles[0][1]), str(children_tiles[1][0]) + ".png")
            x0_y1_path = os.path.join(children_tiles_path, str(children_tiles[0][0]), str(children_tiles[1][1]) + ".png")
            x1_y1_path = os.path.join(children_tiles_path, str(children_tiles[0][1]), str(children_tiles[1][1]) + ".png")
            
            hp.make_dir_if_doesnt_exist(os.path.join(children_tiles_path, str(children_tiles[0][0])))
            hp.make_dir_if_doesnt_exist(os.path.join(children_tiles_path, str(children_tiles[0][1])))
            ...
            if parent_width % 2 != 0:
                parent_tile_resized = parent_tile.resize((parent_width - 1, parent_height - 1))
            else:
                parent_tile_resized = parent_tile
            
            resized_width, resized_height = parent_tile_resized.size
            x0_y0 = parent_tile_resized.crop((0, 0, resized_width / 2, resized_height / 2))
            x1_y0 = parent_tile_resized.crop((resized_width / 2, 0, resized_width, resized_height / 2))
            x0_y1 = parent_tile_resized.crop((0, resized_height / 2, resized_width / 2, resized_height))
            x1_y1 = parent_tile_resized.crop((resized_width / 2, resized_height / 2, resized_width, resized_height))
...
def resize_tiles(zoom, pixel_size):
    ...
    for x in x_list:
        ...
        for y in y_list:
            ...
            # Resize with ImageMagick because the antialias is better
            command = "magick convert " + tile_path + " -resize " + str(pixel_size) + "x" + str(pixel_size) + " " + tile_path
            os.system(command)
