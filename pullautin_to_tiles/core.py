...
def make_max_zoom_index(max_zoom=15, border_filename="border_test.geojson", 
        lidar_index_filename="lidar_index.geojson"):
    """
    """
    ...
    lidar_index = hp.load_geojson_file("index_files\\" + 
        lidar_index_filename)
...
def make_max_zoom_tiles(max_zoom=15, 
        lidar_index_filename="lidar_index.geojson", 
        lidar_index_lambert_93_filename="lidar_index_lambert_93.geojson"):
    """
    """
    ...
    tile_index_max_zoom = load_geojson_file('index_files\\tile_index_' + 
        str(max_zoom) + '.geojson')
    # Load lidar index
    lidar_index = load_geojson_file("index_files\\" + lidar_index_filename)
    # Load lidar index in Lambert 93 projection
    lidar_index = load_geojson_file("index_files\\" + 
        lidar_index_lambert_93_filename)
...
if __name__ == "__main__":
    args = sys.argv[1:]
    if len(args) == 4 and args[0] == "-minzoom" and args[2] == "-maxzoom" \
            and args[1].isdigit() and args[3].isdigit():
        min_zoom = args[1]
        max_zoom = args[3]
        print(min_zoom)
        print(max_zoom)
